...
public class Tokenize {
    ...
    static ArrayList<Token> tokenize(String token) {
        ...
        for (int i = 0; i <= token.length() - 1; i++) {
            if (TOKENTYPES.SPACE.is(String.valueOf(token.charAt(i)))) {
                continue;
            }
            ...
            if (TOKENTYPES.OPERATOR.is(String.valueOf(token.charAt(i)))) {
                ...
                if (i == 0 || TOKENTYPES.OPERATOR.is(String.valueOf(token.charAt(i - 1))) || TOKENTYPES.LBRACKET.is(String.valueOf(token.charAt(i - 1))) && (TOKENTYPES.LITERAL.is(String.valueOf(token.charAt(i + 1)))) && token.charAt(i) == '-') {
                    tokens.add(new Token("Negative", "m"));
                } else {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            if (current.getType().equals("PI") || current.getType().equals("e")) {
                if (i > 0) {
                    ...
                    if (prev.getType().equals("Literal") || prev.getType().equals("RBRACKET") || prev.getType().equals("PIU")) {
